...
package day03 {
  object Day03 {
    ...
    def getShortest(listCoord: Set[Coord]) : Int =
      { for { c <- listCoord } yield c.dist }.min

    def getIntersect(wire1: Set[Coord],wire2: Set[Coord]): Set[Coord] =
      for { i <- wire1 if wire2.contains(i) } yield i
    ...
    def part2(wire1: List[Instruct], wire2: List[Instruct]) : Int = {
      ...
      val ic1 = for { c <- getCoordDist(wire1) if inter.contains(c.c) } yield c
      val ic2 = for { c <- getCoordDist(wire2) if inter.contains(c.c) } yield c
